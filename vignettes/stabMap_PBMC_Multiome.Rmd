...
```{r}
annotation = "celltype"
referenceLabels = colData(experiments(mae)[["rna"]])[colnames(Multiome_sce), annotation]
names(referenceLabels) = colnames(Multiome_sce)

table(referenceLabels)
```

To classify query cells based on a reference dataset we can use the 
`embeddingKNN()` function. We provide the joint embedding generated by 
`stabMap()` and cell type labels for the reference dataset to the
`embeddingKNN()` function. `embeddingKNN()` returns a dataframe with predicted 
labels in the `predicted_labels` column. 

```{r}
knn_out = embeddingKNN(
  stab,
  referenceLabels,
)
```
...
queryLabels = colData(experiments(mae)[["rna"]])[colnames(sce.rna), annotation]
...
stab_umap_ref = stab_umap[colnames(Multiome_sce),]
stab_umap_query = stab_umap[colnames(RNA_sce),]
...
#referenceLabels[match(rownames(stab_umap_ref), names(referenceLabels))]
```
